        ...
        <div className="bg-white p-8 rounded-xl shadow-lg animate-fade-in">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-gray-500">Gestiona a quién se asigna esta evaluación y consulta los resultados.</p>
                </div>
                <button onClick={onBack} className="px-4 py-2 rounded-md text-gray-700 bg-gray-200 hover:bg-gray-300 transition-colors">Volver</button>
            </div>

            <div className="border-b border-gray-200 mb-6">
                <nav className="-mb-px flex space-x-8">
                    <button onClick={() => setActiveTab('group')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'group' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                        Asignar por Grupo
                    </button>
                    <button onClick={() => setActiveTab('individual')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'individual' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                        Asignar Individualmente
                    </button>
                    <button onClick={() => setActiveTab('results')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'results' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            ...
            {activeTab === 'group' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold mb-4">Grupos Asignados ({assessment.assignedGroups?.length || 0})</h3>
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                            {assessment.assignedGroups?.map(({ grade, group }) => (
                                <div key={`${grade}-${group}`} className="flex justify-between items-center p-2 bg-blue-50 border border-blue-200 rounded-md">
                                    <span className="font-medium text-blue-800">{grade} - Grupo {group}</span>
                                    <button onClick={() => handleUnassignGroup(grade, group)} className="text-red-500 hover:text-red-700 text-xs font-bold">QUITAR</button>
                                </div>
                            ))}
                            {(!assessment.assignedGroups || assessment.assignedGroups.length === 0) && <p className="text-gray-500 text-sm">Aún no se han asignado grupos.</p>}
                        </div>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'individual' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto border rounded-lg p-2">
                            {filteredStudents.map(student => (
                                <label key={student.id} className="flex items-center p-2 bg-gray-50 rounded-md cursor-pointer hover:bg-gray-100">
                                    ...
                                    <span className="ml-3 text-sm text-gray-800">{student.name}</span>
                                    <span className="ml-auto text-xs text-gray-500">{student.grade} - {student.group}</span>
                                </label>
                            ))}
                        </div>
                    ...
                    <div>
                        <h3 className="text-lg font-semibold mb-4">Estudiantes Asignados ({individuallyAssignedStudents.length})</h3>
                        <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
                           {individuallyAssignedStudents.map(student => (
                                <div key={student.id} className="flex justify-between items-center p-2 bg-blue-50 border border-blue-200 rounded-md">
                                    ...
                                    <button onClick={() => {
                                        ...
                                        onUpdate(updatedAssessment);
                                    }} className="text-red-500 hover:text-red-700 text-xs font-bold">QUITAR</button>
                                </div>
                            ))}
                            {individuallyAssignedStudents.length === 0 && <p className="text-gray-500 text-sm">Aún no se han asignado estudiantes individualmente.</p>}
                        </div>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'results' && (
                <div>
                    <h3 className="text-lg font-semibold mb-4">Resultados de la Evaluación</h3>
                     <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                        {resultsForThisAssessment.length > 0 ? (
                            resultsForThisAssessment.map(result => {
                                ...
                                return (
                                    <div key={result.id} className="flex items-center justify-between p-3 bg-gray-50 border rounded-lg">
                                        <div className="flex items-center space-x-3">
                                            <img src={student.avatarUrl} alt={student.name} className="w-10 h-10 rounded-full"/>
                                            <div>
                                                <p className="font-semibold text-gray-800">{student.name}</p>
                                                <p className="text-xs text-gray-500">Completado: {new Date(result.completedAt).toLocaleString()}</p>
                                            </div>
                                        </div>
                             ...
                             <p className="text-gray-500 text-center py-8">Ningún estudiante ha completado esta evaluación todavía.</p>
                        )}
                    </div>
                </div>
            )}
            ...
            <div className="space-y-4">
                ...
                {assessments.length === 0 && (
                    <div className="text-center py-16 px-8 bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <p className="mt-1 text-gray-500">Haz clic en "Crear Evaluación con IA" para empezar.</p>
                    </div>
                )}
            </div>
        </div>
