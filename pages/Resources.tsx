...
const ResourceCard: React.FC<{
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
            <div className={`h-24 w-full flex items-center justify-center p-6 ${typeClasses[resource.type]}`}>
                {ICONS[resource.type]}
            </div>
            <div className="p-4 flex flex-col flex-grow">
                <h3 className="font-bold text-gray-800 dark:text-gray-100 truncate">{resource.title}</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{resource.subjectArea}</p>
                <p className="text-sm text-gray-600 dark:text-gray-300 flex-grow">{resource.description}</p>
                 {isDownloaded && !isAiGenerated && (
                    <div className="mt-3 flex items-center text-xs text-green-600 dark:text-green-300 bg-green-100 dark:bg-green-900/50 px-2 py-1 rounded-full w-fit">
                 ...
                 {(isAiGenerated || (isDownloaded && resource.content)) && (
                    <div className="mt-3 flex items-center text-xs text-yellow-800 dark:text-yellow-200 bg-yellow-100 dark:bg-yellow-900/50 px-2 py-1 rounded-full w-fit">
...
const ResourceViewerModal: React.FC<{ resource: Resource; onClose: () => void }> = ({ resource, onClose }) => (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50" onClick={onClose}>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-2xl mx-4 max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4 flex-shrink-0">{resource.title}</h2>
            <div className="prose dark:prose-invert max-w-none overflow-y-auto p-4 bg-gray-50 dark:bg-gray-900 rounded-md border dark:border-gray-700">
                <p className="text-sm italic text-gray-600 dark:text-gray-400 mb-4">{resource.description}</p>
        ...
        <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md space-y-4">
                 <div className="flex justify-between items-center">
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Biblioteca de Recursos</h1>
                    <button
                ...
                <div className="flex items-center justify-end">
                    <label className="flex items-center cursor-pointer">
                        <input type="checkbox" checked={showOnlyDownloaded} onChange={() => setShowOnlyDownloaded(!showOnlyDownloaded)} className="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-primary focus:ring-primary"/>
                        <span className="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">Mostrar solo descargados</span>
                    </label>
                </div>
            </div>
