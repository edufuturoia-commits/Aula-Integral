...
const PriceCard: React.FC<{ plan: string; price: number; period: string; features: string[]; popular?: boolean, annualPrice?: number, onSelect: () => void }> = ({ plan, price, period, features, popular, annualPrice, onSelect }) => {
    ...
    return (
        <div className={`border rounded-xl p-6 flex flex-col ${popular ? 'border-primary shadow-lg' : 'border-gray-200 dark:border-gray-700'}`}>
            ...
            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">{plan}</h3>
            <p className="text-3xl font-extrabold text-primary my-3">{formatter.format(price)}</p>
            <p className="text-gray-500 dark:text-gray-400 font-semibold mb-4 text-sm">{period}</p>
            {annualPrice && <p className="text-xs text-gray-500 dark:text-gray-400 -mt-3 mb-4">Equivalente a {formatter.format(annualPrice)} mensual</p>}
            <ul className="space-y-2 text-gray-600 dark:text-gray-300 text-sm mb-6 flex-grow">
            ...
            <button onClick={onSelect} className={`w-full text-center py-2 px-4 rounded-lg font-semibold transition-colors ${popular ? 'bg-primary text-white hover:bg-primary-focus' : 'bg-gray-100 dark:bg-gray-700 text-primary hover:bg-gray-200 dark:hover:bg-gray-600'}`}>
                Seleccionar Plan
            </button>
        </div>
    )
...
const DemoExpiredModal: React.FC<DemoExpiredModalProps> = ({ onUpgrade, onLogout }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[999]">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl mx-4 animate-zoom-in flex flex-col max-h-[90vh]">
                <div className="p-8 text-center border-b dark:border-gray-700">
                    ...
                    <p className="text-gray-600 dark:text-gray-300 mt-2">¡Gracias por probar AULA INTEGRAL MAYA! Para continuar disfrutando de todas las funcionalidades, por favor elige un plan.</p>
                </div>
                ...
                <div className="p-6 bg-gray-50 dark:bg-gray-900/50 border-t dark:border-gray-700 text-center">
                    <p className="text-sm text-gray-600 dark:text-gray-300">¿No estás listo para comprar? Puedes cerrar sesión y volver más tarde.</p>
