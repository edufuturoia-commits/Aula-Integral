...
const AttendanceTaker: React.FC<AttendanceTakerProps> = ({ students, isOnline }) => {
    ...
    const summary = useMemo(() => {
        ...
        attendance.forEach(status => {
            if (status === AttendanceStatus.ABSENT || status === AttendanceStatus.TARDY) {
                counts[status]++;
            }
        });
        
        const presentCount = students.length - counts[AttendanceStatus.ABSENT] - counts[AttendanceStatus.TARDY];

        return {
            ...counts,
            [AttendanceStatus.PRESENT]: presentCount >= 0 ? presentCount : 0,
        };
    }, [attendance, students]);
                        ...
                        return (
                            <div key={student.id} className={`p-3 rounded-lg flex items-center justify-between border-l-4 ${statusClasses.border} ${statusClasses.bg}`}>
                                ...
                                <div className="flex gap-2">
                                    {[AttendanceStatus.PRESENT, AttendanceStatus.ABSENT, AttendanceStatus.TARDY].map(status => {
