...
const StudentSelector: React.FC<{
    ...
    return (
        <div className="absolute top-full left-0 mt-1 w-full max-w-sm bg-white shadow-lg rounded-lg border z-10 p-4">
                ...
                {filteredStudents.map(student => (
                    <li key={student.id}>
                        <label className="flex items-center p-2 hover:bg-gray-100 rounded-md cursor-pointer">
                            ...
                            />
                            <span className="ml-3 text-sm">{student.name} ({student.grade}-{student.group})</span>
                        </label>
                    </li>
                ))}
            ...
            <div className="flex justify-end gap-2 mt-4">
                <button onClick={onCancel} className="text-sm px-3 py-1 rounded-md bg-gray-200 hover:bg-gray-300">Cancelar</button>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col">
          <div className="flex justify-between items-center mb-6 flex-shrink-0">
            <h2 className="text-2xl font-bold text-gray-800">Importar Acudientes y Vincular Estudiantes</h2>
            <button onClick={onClose} className="text-gray-500 hover:text-gray-800 text-3xl">&times;</button>
          </div>
          
          {step === 1 && (
            <div>
              <p className="text-gray-600 mb-4">Sube un listado de acudientes en formato PDF o Excel. La IA extraerá sus datos para que luego puedas vincularlos a los estudiantes correspondientes.</p>
              <input type="file" onChange={handleFileChange} accept=".pdf,.xls,.xlsx" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20"/>
              {file && <p className="text-sm text-gray-500 mt-2">Archivo: <strong>{file.name}</strong></p>}
              {error && <p className="text-red-600 text-sm text-center mt-2">{error}</p>}
              <div className="flex justify-end space-x-4 mt-8">
                 <button onClick={onClose} className="px-6 py-2 rounded-md text-gray-700 bg-gray-200 hover:bg-gray-300">Cancelar</button>
          ...
          {step === 2 && (
            <div className="flex-1 flex flex-col overflow-hidden">
                <p className="text-gray-600 mb-4 flex-shrink-0">Revisa los datos y asigna uno o más estudiantes a cada acudiente.</p>
                            ...
                            return (
                                <div key={index} className="p-4 border rounded-lg bg-gray-50 space-y-2">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <input type="text" value={guardian.name} onChange={(e) => handleGuardianDataChange(index, 'name', e.target.value)} className="w-full p-2 border border-gray-300 rounded-md bg-white text-gray-900 focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Nombre"/>
                                        <input type="text" value={guardian.id} onChange={(e) => handleGuardianDataChange(index, 'id', e.target.value)} className="w-full p-2 border border-gray-300 rounded-md bg-white text-gray-900 focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="ID/Cédula"/>
                                        <div className="relative">
                                            <button onClick={() => setLinkingGuardianIndex(linkingGuardianIndex === index ? null : index)} className="w-full bg-blue-100 text-blue-700 font-semibold py-2 px-4 rounded-lg hover:bg-blue-200">
                                    ...
                                    {linkedStudents.length > 0 && (
                                        <div className="text-sm text-gray-700">
                                            <strong>Estudiantes asignados:</strong> <span className="text-primary font-medium">{linkedStudents.join(', ')}</span>
                                        </div>
                                    )}
                                </div>
                            )
                ...
                <div className="flex justify-between items-center mt-6 flex-shrink-0">
                    <button onClick={() => setStep(1)} className="px-6 py-2 rounded-md text-gray-700 bg-gray-200 hover:bg-gray-300">Atrás</button>
